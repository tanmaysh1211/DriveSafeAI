...
public class TripService {
    ...
    private String callFlaskForHeatmap(List<Map<String, String>> rows, Long userId) {
        ...
        ResponseEntity<Map> response = restTemplate.postForEntity(flaskMlUrl + "/generate-heatmap", request, Map.class);
    ...
    public TripUploadResponse processTrip(MultipartFile file, String email) throws Exception {
        ...
        if (stats.startLat != 0 && stats.startLng != 0) {
            WeatherService.WeatherInfo info = weatherService.getWeatherAtLocation(stats.startLat, stats.startLng);
    ...
    public List<TripSummaryResponse> getTripsByUser(Long userId, int limit) {
        return tripRepository.findTopNByUserIdOrderByCreatedAtDesc(userId, limit).stream()
    ...
    private double callFlaskForScore(TelemetryStats stats, String weather, boolean isDaytime) {
        try {
            ...
            ResponseEntity<Map> response = restTemplate.postForEntity(flaskMlUrl + "/predict", request, Map.class);
    ...
    private void createSampleTrip(User user, double score, String risk,double maxSpeed, double distance, String route) {
        Trip trip = new Trip();
        trip.setUser(user);
    ...
    public TripUploadResponse processSimulatedTrip(String email, String profile, String route) throws Exception {
        ...
        ResponseEntity<Map> response = restTemplate.postForEntity(flaskMlUrl + "/simulate", request, Map.class);

        if (response.getStatusCode() != HttpStatus.OK || response.getBody() == null) {
            throw new RuntimeException("Flask simulation failed");
        }

        List<Map<String, Object>> rawRows = (List<Map<String, Object>>) response.getBody().get("rows");
    ...
    private String callFlaskForMap(List<Map<String, String>> rows, Long userId) {
        ...
        ResponseEntity<Map> response = restTemplate.postForEntity(flaskMlUrl + "/generate-map", request, Map.class);
