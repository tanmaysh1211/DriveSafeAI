...
public class TripController {
    ...
    public ResponseEntity<?> uploadTrip(
        ...
        if (file.isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of("message", "CSV file is empty"));
        }

        String contentType = file.getContentType();
        if (contentType == null || !contentType.contains("csv")) {
            if (!Objects.requireNonNull(file.getOriginalFilename()).endsWith(".csv")) {
                return ResponseEntity.badRequest().body(Map.of("message", "Only CSV files are accepted"));
            }
        }
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("message", "Failed to process trip: " + e.getMessage()));
        }
    }

    @PostMapping("/simulate")
    public ResponseEntity<?> simulateTrip( @RequestBody Map<String, String> body, Authentication auth) {
        try {
            String profile = body.getOrDefault("profile", "average");
            String route = body.getOrDefault("route", "bangalore_electronic_city");
            TripUploadResponse result = tripService.processSimulatedTrip( auth.getName(), profile, route);
    ...
    @GetMapping("/{userId}")
    public ResponseEntity<List<TripSummaryResponse>> getUserTrips( @PathVariable Long userId, @RequestParam(defaultValue = "10") int limit) {
    ...
    public ResponseEntity<?> getAIAnalysis(@PathVariable Long tripId) {
        ...
        if (analysis == null || analysis.isBlank()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", "AI analysis not available for this trip"));
        }
    ...
    public ResponseEntity<?> getMapUrl(@PathVariable Long tripId) {
        ...
        if (mapUrl == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", "Map not generated for this trip"));
        }
    ...
    public ResponseEntity<?> deleteTrip(
        ...
        if (!deleted) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of("message", "Trip not found or not authorized"));
        }
